...
class PeekActivity : AppCompatActivity() {
    ...
    private fun newPeek() {
        ...
        save.setOnClickListener {
            var c = this
            GlobalScope.launch{
                var datalist = StreetPassRecordStorage(c).getAllRecords() //List<StreetPassRecord>
                val file = File(c.filesDir, "bluetooth_data.txt")
                file.delete();
                // /data/data/io.bluetrace.opentrace/files/bluetooth_data.txt
                var data = datalist.get(1)
                for (data in datalist) {
                        file.appendText(Utils.getDate(data.timestamp)+","+data.rssi+","+data.modelC+","+data.modelP+"\n")
                }
              }
           }
